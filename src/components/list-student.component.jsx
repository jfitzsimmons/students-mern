...
const ListBar = (props) => (
  <tr>
    ...
    <td>
      {props.student.student_verification ? (
    ...
    <td>
      <Link to={`/edit/${props.student._id}`}>
        <AiOutlineEdit />
      </Link>
      <Link to={`/delete/${props.student._id}`}>
        <AiOutlineDelete />
      </Link>
    </td>
  </tr>
...
const ListStudent = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await axios('http://localhost:4000/all_student/');
